...
try {
    ...
    // 3) Excel column to category code mapping
    $excel2kod = [
        'C' => 'AKTIF_FTR',
        'D' => 'AKTIF_DYT',
        'E' => 'ARA_VEREN_FTR',
    ...
    for ($r = 3; $r <= $maxRow; ++$r) {
        ...
        // c) Process categories (flags in columns C-O)
        foreach ($excel2kod as $col => $kod) {
            $cellValue = cell($sheet, $col, $r);
            if ($cellValue === '1' && isset($katRows[$kod])) {
                $stmtInsLink->execute([
                    ':did' => $danId,
                    ':kid' => $katRows[$kod],
                    ':pid' => $currentStaffId
        ...
        if ($rawNotes !== '') {
            ...
            foreach ($lines as $line) {
                ...
                // Try to extract date and staff code
                if (preg_match('/^(\d{2}\.\d{2}(?:\.\d{2,4})?)[\s\/\-]+([A-Z]{2})[\s\-]+(.+)$/u', $line, $m)) {
                    ...
                    $staffId = $kod2pid[$staffCode] ?? $currentStaffId;
