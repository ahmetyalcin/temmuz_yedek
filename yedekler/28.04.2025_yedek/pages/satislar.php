                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <?php if ($satis['odenmemis_taksit_sayisi'] > 0): ?>
                                        <button type="button" class="btn btn-sm btn-success" 
                                                onclick="showPaymentModal('<?php echo $satis['id']; ?>')">
                                            <i class="fas fa-plus"></i> Ödeme
                                        </button>
                                    <?php endif; ?>
...
function addAppointmentSlot() {
    ...
    // Determine if this is a gift session
    const currentIndex = selectedSlots.length;
    const is_gift = currentIndex >= regularSessions;
    let evaluation_type = null;
    
    if (is_gift) {
        if (currentIndex === regularSessions) {
...
function updateSelectedSlots() {
    ...
    selectedSlots.forEach((slot, index) => {
        ...
        tr.innerHTML = `
            ...
            <td>
                ${slot.is_gift ? '<span class="badge bg-warning">Hediye</span>' : ''}
                ${slot.evaluation_type === 'initial' ? '<span class="badge bg-info">İlk Değerlendirme</span>' : ''}
                ${slot.evaluation_type === 'final' ? '<span class="badge bg-info">Son Değerlendirme</span>' : ''}
            </td>
            <td>
