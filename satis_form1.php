...
$satis_turleri = getSatisTurleri(); // Tablodan çek
$odeme_turleri = getOdemeTurleri(); // YENİ: Ödeme türleri
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        // Satış kaydı - TEMİZLENMİŞ PARAMETRELERle  
        $satis_id = satisEkle(
            ...
            $_POST['notlar'],               // notlar
            intval($_POST['satis_turu_id']), // Satış türü ID
            $current_user_id,               // İşlem yapan kullanıcı ID (arka planda)
            intval($_POST['odeme_turu_id']) // Ödeme türü ID
        );
            ...
            foreach ($_POST['taksitler'] as $taksit) {
                if (!empty($taksit['tutar']) && !empty($taksit['vade_tarihi'])) {
                    taksitEkle(
                        ...
                        $taksit['vade_tarihi'],
                        intval($_POST['odeme_turu_id']) // Aynı ödeme türü
                    );
                }
            }
        }
                            ...
                            <div class="row g-3">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <select name="odeme_turu_id" class="form-select" required>
                                                ...
                                                <?php foreach($odeme_turleri as $odeme): ?>
                                                    <option value="<?= $odeme['id'] ?>" title="<?= htmlspecialchars($odeme['aciklama']) ?>">
                                ...
                                <div class="col-12">
                                    <div class="row g-3">
                                        <div class="col-md-12">
                                            <label>Notlar</label>
                                            <textarea name="notlar" class="form-control" rows="5" required="" placeholder=""></textarea>
                                        </div>
                                    </div>
                                </div>
