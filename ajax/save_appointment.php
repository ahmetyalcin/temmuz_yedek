...
try {
    ...
    error_log("SAVE_APPOINTMENT DEBUG:");
    error_log("ID: " . ($id ?? 'null'));
    error_log("Danisan ID: " . ($danisan_id ?? 'null'));
    error_log("Personel ID: " . ($personel_id ?? 'null'));
    error_log("Randevu Tarihi: " . ($randevu_tarihi ?? 'null'));
    error_log("Room ID: " . ($room_id ?? 'null'));
    error_log("Satis ID: " . ($satis_id ?? 'null'));
    ...
    if ($satis_id) {
        ...
        if ($satis) {
            ...
            $seans_turu_id = $satis['hizmet_paketi_id']; // Seans türü ID'sini al
            
            $is_gift = $used_sessions >= $regular_sessions ? 1 : 0;
            
            error_log("DEBUG - Satis bilgisi bulundu. Seans_turu_id: " . $seans_turu_id);
        } else {
            error_log("DEBUG - Satis bilgisi bulunamadı!");
        }
    }

    // Eğer seans_turu_id bulunamazsa hata ver
    if (!$seans_turu_id) {
    ...
    if ($id) {
        ...
        error_log("DEBUG - Güncelleme başarılı: " . ($success ? 'true' : 'false'));
    } else {
        // Yeni kayıt - UUID oluştur
        $new_id = generateUUID();
        
        error_log("DEBUG - Yeni UUID oluşturuluyor: " . $new_id);
        ...
        error_log("DEBUG - Ekleme başarılı: " . ($success ? 'true' : 'false'));
        error_log("DEBUG - Eklenen ID: " . $new_id);
    }
    ...
    error_log("SAVE_APPOINTMENT ERROR: " . $e->getMessage());
    echo json_encode([
        'success' => false,
