...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT s.*, st.seans_adet, st.ad as seans_turu, st.evaluation_interval,
    ...
    $is_functional = strpos(strtolower($satis['seans_turu']), 'fonksiyonal') !== false;
    $is_pilates = strpos(strtolower($satis['seans_turu']), 'pilates') !== false;
    $evaluation_interval = $is_functional ? 6 : ($is_pilates ? 8 : null);
    ...
    if (!empty($to_add)) {
        $stmt = $pdo->prepare("
            INSERT INTO randevular (
                ...
                evaluation_type, evaluation_notes, evaluation_number
            ) VALUES (
        ...
        foreach ($to_add as $datetime => $apt) {
            ...
            // Handle evaluation sessions
            $session_number = $appointment_count + 1;
            
            if ($evaluation_interval) {
                if ($session_number === 1) {
                    $evaluation_type = 'initial';
                    $evaluation_number = ++$evaluation_count;
                } elseif ($session_number === $total_sessions) {
                    $evaluation_type = 'final';
                    $evaluation_number = ++$evaluation_count;
                } elseif ($evaluation_interval && $session_number % $evaluation_interval === 0) {
            ...
            $stmt->execute([
                ...
                $is_gift,
                $evaluation_type,
                null, // evaluation_notes starts as null
                $evaluation_number
            ]);
    ...
    ");
    $stmt->execute([$data['satis_id']]);
    $updated_appointments = $stmt->fetchAll(PDO::FETCH_ASSOC);
